...
export default async function main(uiBuilder: UIBuilder) {
  uiBuilder.form((form: any) => ({
    ...
    let { table_Source_List, table_Target, field_Target } = values;
    ...
    uiBuilder.showLoading('开始准备合并数据...');
    ...
    for (const table_Source of table_Source_List) {
      ...
      while (hasMore) {
        ...
        const source_recordValueList = await table_source.getRecords({ pageSize: pageSize, pageToken: pageToken });
